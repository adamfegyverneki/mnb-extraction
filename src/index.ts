...
program
  .command("schedule")
  .description("Run fetch on a cron schedule (default: 12:00 Mon-Fri, Europe/Budapest)")
  .option(
    "-c, --cron <expression>",
    "Cron expression",
    "0 12 * * 1-5",
  )
  .action((options: { cron: string }) => {
    console.log(
      `Scheduler started: "${options.cron}" (timezone: Europe/Budapest)`,
    );
...
program
  .command("list")
  .description("List saved snapshot dates")
  .action(async () => {
